...
function getCapacityMetrics(course) {
  ...
  const capacity = Number.isFinite(seats.capacity) ? seats.capacity : null;
  const taken = Number.isFinite(seats.taken) ? seats.taken : null;
  const seatsRemaining = Number.isFinite(seats.remaining)
    ? seats.remaining
    : capacity !== null && taken !== null
      ? Math.max(capacity - taken, 0)
      : null;
  const waitlist = Number.isFinite(seats.waitlist) ? seats.waitlist : 0;

  // Mirrors the backend demand bands: full or heavily waitlisted -> high,
  // nearly full or queued -> moderate, otherwise open.
  const fillRatio = capacity ? (taken ?? 0) / capacity : null;
  const demandRank =
    fillRatio === null
      ? 0
      : fillRatio >= 1 || waitlist >= 10
        ? 3
        : fillRatio >= 0.8 || waitlist > 0
          ? 2
          : 1;
  ...
  if (stateKind === "approved") {
    ...
    if (course.preview.uiVariant === "lottery") {
      ...
      summary = {
        ...
        // The backend distinguishes "all seats taken" from "free seats held
        // for the existing queue"; surface its actual reason.
        reasonText: firstReason || "Seats are full, but the waitlist is still open.",
