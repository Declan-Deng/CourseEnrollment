...
export function cancelStudentRequest(snapshot, offeringId, { actor } = {}) {
  ...
  const offering = getOfferingMutable(nextSnapshot, offeringId);
  const isPendingLotteryEntry = request.status === "lotteryQueued";

  if (!isPendingLotteryEntry && !isWindowCurrentlyOpen(offering?.requestWindow, referenceDate)) {
    return {
      snapshot,
...
export function resetStudentState(snapshot, seedSnapshot, { actor } = {}) {
  ...
  const displacedOfferingIds = new Set();
  ...
  for (const offering of nextSnapshot.offerings) {
    ...
    if (!currentEnrollmentIds.has(offering.id) && seedEnrollmentIds.has(offering.id)) {
      if (offering.seatsTaken < offering.capacity) {
        ...
        // The shared seat was legitimately taken (e.g. by a waitlist
        // promotion) while this student was away from their seed state:
        // restoring the enrollment would oversell the offering, so the
        // student rejoins the queue instead.
        displacedOfferingIds.add(offering.id);
        offering.waitlistCount += 1;
      }
    }
  ...
  if (displacedOfferingIds.size > 0) {
    seedState.enrollments = seedState.enrollments.filter(
      (enrollment) => !displacedOfferingIds.has(enrollment.offeringId),
    );

    for (const offeringId of displacedOfferingIds) {
      seedState.studentMeta.recordSequence = Number.isFinite(seedState.studentMeta.recordSequence)
        ? seedState.studentMeta.recordSequence + 1
        : 1;
      seedState.requests.unshift({
        id: `req-${seedState.student.id}-${seedState.studentMeta.recordSequence}`,
        studentId: seedState.student.id,
        offeringId,
        status: "waitlist",
        active: true,
        submittedAt: formatRecordTimestamp(),
        message: "Your previous seat was taken while the demo state was reset, so you are back on the waitlist.",
        resolution: null,
      });
    }
  }
