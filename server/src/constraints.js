...
export function evaluateEnrollment(state, courseId) {
  ...
  if (course.allocationPolicy === "firstComeFirstServed") {
    if (seatsRemaining > 0 && course.seats.waitlist === 0) {
    ...
    if (seatsRemaining > 0) {
      return createAccepted(
        "Waitlisted students have priority; you can join the queue.",
        "waitlist",
        [
          "No rule conflict was found, but earlier waitlist requests are still being processed for the remaining seats.",
          `${formatCountLabel(course.seats.waitlist, "student")} ${course.seats.waitlist === 1 ? "is" : "are"} already waiting ahead of new requests.`,
        ],
        [],
        "waitlist",
      );
