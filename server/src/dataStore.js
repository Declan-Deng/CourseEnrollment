...
import {
  ...
  listAdminRequests,
  previewRequestResolution,
  promoteWaitlistedRequest,
  resolveRequestForAdmin,
} from "./adminRequestService.js";
...
async function buildAdminAggregateState(activeRepository) {
  ...
  return {
    ...baseSnapshot,
    students: snapshots.map((snapshot) => snapshot.student).filter(Boolean),
...
async function collectWaitlistedRequests(activeRepository, offeringId) {
  const studentIds = await activeRepository.listStudentIds();
  const candidates = [];
  ...
  for (const studentId of studentIds) {
    ...
    for (const request of state.requests) {
      if (request.offeringId === offeringId && request.active && request.status === "waitlist") {
        candidates.push({ studentId, request });
      }
    }
  }
  ...
  return candidates.sort(
    (left, right) =>
      String(left.request.submittedAt ?? "").localeCompare(String(right.request.submittedAt ?? "")) ||
      String(left.request.id).localeCompare(String(right.request.id)),
  );
}

// FCFS queue semantics: whenever seats free up while students are waiting,
// the earliest waitlisted entry takes the seat instead of leaving it open for
// a newcomer. Seeded waitlist counts represent students who queued before any
// tracked request, so untracked entries consume seats first (anonymously) and
// tracked requests are promoted once the seeded part of the queue is drained.
async function rebalanceFcfsWaitlist(activeRepository, offeringId) {
  const promotionGuard = 1000;
  const conflictRetryLimit = 5;
  let conflictRetries = 0;

  for (let iteration = 0; iteration < promotionGuard; iteration += 1) {
    const offering = await activeRepository.offeringRepository.getById(offeringId);

    if (
      !offering ||
      offering.allocationPolicy !== "firstComeFirstServed" ||
      offering.capacity - offering.seatsTaken <= 0 ||
      offering.waitlistCount <= 0
    ) {
      return;
    }

    const candidates = await collectWaitlistedRequests(activeRepository, offeringId);
    const untrackedCount = Math.max(offering.waitlistCount - candidates.length, 0);

    try {
      if (untrackedCount > 0 || candidates.length === 0) {
        const nextOffering = {
          ...offering,
          seatsTaken: offering.seatsTaken + 1,
          waitlistCount: offering.waitlistCount - 1,
        };
        await activeRepository.updateOfferings([{ offering: nextOffering, expectedVersion: offering.version }]);
        await activeRepository.auditRepository.append([
          createAuditEvent({
            ...
            targetId: offeringId,
            before: { offering },
            after: { offering: { ...nextOffering, version: offering.version + 1 } },
          }),
        ]);
      } else {
        const queueHead = candidates[0];
        const state = await activeRepository.getState(queueHead.studentId);
        const nextSnapshot = promoteWaitlistedRequest(state, queueHead.request.id);
        await persistSnapshotTransition(activeRepository, state, nextSnapshot, queueHead.studentId);
      }
    } catch (error) {
      // Another writer touched the offering mid-promotion: re-read and retry
      // instead of surfacing a conflict for a drop/patch that already committed.
      if (error instanceof RepositoryConflictError && conflictRetries < conflictRetryLimit) {
        conflictRetries += 1;
        continue;
      }
  ...
  console.warn(`Waitlist rebalance for ${offeringId} stopped after ${promotionGuard} promotions.`);
}

async function rebalanceFcfsWaitlistSafely(activeRepository, offeringId) {
  try {
    await rebalanceFcfsWaitlist(activeRepository, offeringId);
  } catch (error) {
    // The triggering mutation already committed; a failed rebalance must not
    // turn it into an error response. The next seat change retries naturally.
    console.warn(`Waitlist rebalance for ${offeringId} was interrupted: ${error.message}`);
  }
}

async function rebalanceAllFcfsWaitlists(activeRepository) {
  const offerings = await activeRepository.offeringRepository.list();
  ...
  for (const offering of offerings) {
    if (
      ...
      await rebalanceFcfsWaitlistSafely(activeRepository, offering.id);
    }
  }
}
...
export async function previewAdminOverrideImpact(payload, options = {}) {
  ...
  const { repository: activeRepository, state } = await getStateContext({ ...options, studentId });
  ...
  const knownStudentIds = await activeRepository.listStudentIds();
  const studentKnown = knownStudentIds.includes(studentId);
...
export async function dropCourse(offeringId, options = {}) {
  return withWriteLock(async () => {
    const decision = await saveStateWithRetry((state) => {
      const actor = resolveActor(options, { type: "student", id: state.student.id });
    ...
    if (decision?.ok) {
      await rebalanceFcfsWaitlistSafely(await getRepository(), offeringId);
    }
  ...
  return withWriteLock(async () => {
    ...
    await saveStateWithRetry((state) => {
      ...
      return {
        ...
        value: resetSnapshot.student.id,
      };
    }, { studentId });

    // A student reset can free seats on shared offerings; waiting students
    // must take those seats before newcomers see them as open.
    await rebalanceAllFcfsWaitlists(activeRepository);
    const freshState = await activeRepository.getState(studentId);
    return buildBootstrapResponse(freshState);
  });
}
...
export async function updateAdminOffering(offeringId, patch, options = {}) {
  return withWriteLock(async () => {
    const value = await saveStateWithRetry((state) => {
    ...
    const activeRepository = await getRepository();
    await rebalanceFcfsWaitlistSafely(activeRepository, offeringId);
    const refreshedOffering = await activeRepository.offeringRepository.getById(offeringId);

    return refreshedOffering ? { ...value, offering: refreshedOffering } : value;
  });
}
...
export async function listAdminRequestView(options = {}) {
  ...
  return snapshots.flatMap((snapshot) => {
    ...
    return listAdminRequests(snapshot, options.filters ?? {}).map((request) => ({
      ...request,
      student: snapshot.student,
      allowedResolutions: getAllowedResolutionActions(offeringById.get(request.offeringId) ?? null, request),
    }));
  });
}
