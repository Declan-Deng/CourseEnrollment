...
function getRequestNextStep(status, semester, course) {
  ...
  const nextStepByStatus = {
    ...
    lotteryQueued: () =>
      course.requestOpen
        ? `The programme office resolves the lottery pool after ${semester.keyDates?.lotteryPublish ?? "the draw window"}. Keep an alternative ready; you can withdraw while it is pending.`
        : `The programme office is resolving the lottery pool. You can still withdraw this request until an outcome is published.`,
...
export function createRequestRecordView(request, course, semester) {
  return {
    ...
    withdrawable: request.active && (Boolean(course.requestOpen) || request.status === "lotteryQueued"),
