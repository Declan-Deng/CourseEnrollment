...
function buildGeneratedSeatOccupants(offeringId, count, trackedCount) {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const studentId = `3036${studentSuffix}`;

    return {
      studentId,
      studentName: getSimulatedStudentName(studentId),
...
function buildSeatOccupants(snapshot, offering) {
  const profileById = new Map((snapshot.students ?? []).map((student) => [student.id, student]));
  const trackedOccupants = snapshot.enrollments
    ...
    .map((enrollment) => ({
      ...
      studentName: profileById.get(enrollment.studentId)?.name ?? getSimulatedStudentName(enrollment.studentId),
