...
test("admin request resolution preview can describe an invalid approval without returning 409", async () => {
  ...
  assert.match(preview.body.headline, /full/i);
});

test("admin request resolution endpoint resolves lottery requests manually", async () => {
  const requests = await requestJson("/api/admin/requests?active=true", {
    headers: {
      "x-actor-type": "staff",
      "x-actor-id": "staff-http-lottery-resolve-001",
    },
  });
  const lotteryRequest = requests.body.find((item) => item.offeringId === "COMP7906-B-S2");

  assert.ok(lotteryRequest);
  assert.deepEqual(lotteryRequest.allowedResolutions, ["approve", "reject", "waitlist", "manual-close"]);

  const fullApproval = await requestJson(`/api/admin/requests/${encodeURIComponent(lotteryRequest.id)}/resolve`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "x-actor-type": "staff",
    ...
    body: JSON.stringify({
      action: "approve",
      note: "Lottery winner drawn by the programme office.",
    }),
  });

  assert.equal(fullApproval.response.status, 409);
  assert.equal(fullApproval.body.ok, false);
  assert.match(fullApproval.body.headline, /full/i);

  const rejection = await requestJson(`/api/admin/requests/${encodeURIComponent(lotteryRequest.id)}/resolve`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "x-actor-type": "staff",
    ...
    body: JSON.stringify({
      action: "reject",
      note: "Lottery pool resolved manually; not drawn this round.",
    }),
  });

  assert.equal(rejection.response.status, 200);
  assert.equal(rejection.body.ok, true);
  assert.equal(rejection.body.request.status, "rejected");
  assert.equal(rejection.body.request.active, false);
});
import test from "node:test";
import assert from "node:assert/strict";
import { startServer } from "../src/index.js";
